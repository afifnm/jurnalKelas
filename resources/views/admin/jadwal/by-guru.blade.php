...
@section('breadcrumb')
    ...
    <i data-lucide="chevron-right" class="w-3 h-3"></i><span>Per Guru</span>
@endsection
...
<div class="flex items-center justify-between mb-5">
    ...
    <a href="{{ route('admin.users.index') }}" class="btn-secondary text-sm">
        <i data-lucide="arrow-left" class="w-4 h-4"></i> Kembali ke Pengguna
    </a>
</div>

<!-- Filter -->
<form method="GET" class="flex items-center gap-2 mb-5">
    <select name="tahun_ajaran_id" class="input-field w-56 text-sm flex-shrink-0">
    ...
    <a href="{{ route('admin.jadwal.by-guru') }}" class="btn-secondary text-sm flex-shrink-0">
        <i data-lucide="x" class="w-4 h-4"></i>
    </a>
    @endif
    <span class="ml-auto text-xs text-slate-400 dark:text-zinc-500 flex items-center gap-1.5">
        <span class="w-2.5 h-2.5 rounded bg-amber-300 dark:bg-amber-700 inline-block"></span>
        Hari ini: <span class="font-medium text-amber-600 dark:text-amber-400">{{ $namaHari[$hariIni] ?? '' }}</span>
    </span>
</form>
...
<div x-data="{ activeGuru: {{ $guruId ?? $guruList->first()->id }} }" class="space-y-5">
    ...
    <div class="flex flex-wrap gap-2">
        ...
        <button
            ...
            class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all relative">
            {{ $guru->nama }}
            @if($mengajarHariIni)
    ...
    <div x-show="activeGuru === {{ $guru->id }}" x-transition.opacity.duration.150ms>
        ...
        <div class="card overflow-hidden">
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-zinc-700/50">
                ...
                <a href="{{ route('admin.jurnal.index', ['guru_id' => $guru->id]) }}"
                   class="btn-secondary text-xs py-1.5">
                    <i data-lucide="notebook-text" class="w-3.5 h-3.5"></i> Lihat Jurnal
                </a>
            </div>
