    ...
    <form method="GET" class="flex flex-wrap items-center gap-2 mb-4">
        <div class="relative flex-1 min-w-[160px]">
            ...
            <input type="text" name="search" value="{{ request('search') }}"
                class="input-field py-1.5 pl-8 text-sm w-full">
        </div>
        <select name="role" class="input-field py-1.5 text-sm flex-1 min-w-[120px]">
    ...
    <div class="card overflow-hidden">
        ...
        <div class="divide-y divide-slate-100 dark:divide-zinc-700/50 md:hidden">
            ...
            <div class="p-4 {{ $user->trashed() ? 'opacity-50' : '' }}">
                    ...
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="flex items-center gap-1.5 flex-wrap mt-0.5">
                                <code class="text-[10px] bg-slate-100 dark:bg-zinc-700 px-1.5 py-0.5 rounded text-slate-600 dark:text-zinc-300">{{ $user->username }}</code>
                                <span class="badge {{ match($role) { 'admin' => 'bg-red-100 dark:bg-red-950/40 text-red-700 dark:text-red-400', 'ks' => 'bg-purple-100 dark:bg-purple-950/40 text-purple-700 dark:text-purple-400', 'guru' => 'bg-blue-100 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400', default => 'bg-slate-100 text-slate-600' } }}">{{ ucfirst($role) }}</span>
                                @if($user->trashed())
                ...
                @if($user->email || $user->no_hp)
                <p class="text-xs text-slate-400 dark:text-zinc-500 mb-2 pl-12">
                    @if($user->email){{ $user->email }}@endif
                    @if($user->email && $user->no_hp) · @endif
                    @if($user->no_hp){{ $user->no_hp }}@endif
                </p>
                @endif
