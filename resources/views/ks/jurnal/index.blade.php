                    ...
                    <div class="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        <div class="min-w-0">
                            <p class="font-semibold text-slate-700 dark:text-slate-200 text-sm truncate">
                                <a href="{{ route('ks.jadwal.by-guru', ['guru_id' => $j->guru->id]) }}" class="hover:underline hover:text-blue-600">{{ $j->guru->nama }}</a>
                            </p>
                            <p class="text-xs text-slate-400">
                                <a href="{{ route('ks.jadwal.by-kelas', ['kelas_id' => $j->kelas->id]) }}" class="hover:underline hover:text-blue-600">{{ $j->kelas->nama }}</a> · {{ $j->mapel->nama }}
                            </p>
                        </div>
                    </div>
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                        ...
                        <td class="px-4 py-3.5">
                            <p class="font-medium text-slate-700 dark:text-slate-200">
                                <a href="{{ route('ks.jadwal.by-kelas', ['kelas_id' => $j->kelas->id]) }}" class="hover:underline hover:text-blue-600">{{ $j->kelas->nama }}</a>
                            </p>
...
function jurnalViewManager() {
    return {
        ...
        async viewDetail(id) {
            ...
            const res = await fetch(`/ks/jurnal/${id}`, {
                cache: 'no-store',
