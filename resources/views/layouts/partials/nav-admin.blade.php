...
{!! navItem(route('admin.jadwal.index'), 'calendar-clock', 'Jadwal', str_starts_with($currentRoute, 'admin.jadwal')) !!}
{!! navItem(route('admin.jurnal.index'), 'notebook-text', 'Jurnal Guru', str_starts_with($currentRoute, 'admin.jurnal')) !!}

<p class="px-3 mt-4 mb-1.5 text-[10px] font-semibold text-slate-400 dark:text-zinc-600 uppercase tracking-widest">Pengaturan</p>

{!! navItem(route('admin.tahun-ajaran.index'), 'calendar-range', 'Tahun Ajaran', str_starts_with($currentRoute, 'admin.tahun-ajaran')) !!}
{!! navItem(route('admin.sekolah.index'), 'building-2', 'Identitas Sekolah', str_starts_with($currentRoute, 'admin.sekolah')) !!}
