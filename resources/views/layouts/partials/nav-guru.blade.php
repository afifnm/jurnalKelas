@php
    ...
    $isJadwalActive = str_starts_with($currentRoute, 'guru.jadwal');
@endphp
...
<a href="{{ route('guru.jadwal.index') }}"
   class="flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all {{ str_starts_with($currentRoute, 'guru.jadwal') ? 'bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400' : 'text-slate-600 dark:text-zinc-400 hover:bg-slate-50 dark:hover:bg-zinc-800 hover:text-slate-800 dark:hover:text-zinc-100' }}">
